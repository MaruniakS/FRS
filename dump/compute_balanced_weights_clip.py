...
def balance_weights(weights):
    """
    Balances SHAP weights using Adaptive Redistribution.
    ...
    mean, std = np.mean(weight_values), np.std(weight_values)

    # Apply soft clipping
    max_threshold = mean + 2 * std
    weight_values = np.clip(weight_values, None, max_threshold)
    ...
    # Identify dominant features (above 0.8)
    dominant_indices = weight_values > 0.8
    excess_weight = np.sum(weight_values[dominant_indices] - 0.8)

    # Cap dominant features at 0.8
    weight_values[dominant_indices] = 0.8

    # Redistribute excess weight to remaining features
    non_dominant_indices = ~dominant_indices
    num_non_dominant = np.sum(non_dominant_indices)
    if num_non_dominant > 0:
        weight_values[non_dominant_indices] += excess_weight / num_non_dominant  # Spread excess weight

    return dict(zip(weights.keys(), weight_values))

def compute_shap_weights(model, X, class_index):
    """
    Computes SHAP-based feature weights for a specific class using KernelExplainer with Adaptive Redistribution.
    ...
    # Normalize weights using Redistribution Strategy
